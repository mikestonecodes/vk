// Mega Splat - Blur Horizontal Pass
...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    for (int dx = -radius; dx <= radius; ++dx) {
        int2 sample_coord = int2(coord) + int2(dx, 0);

        // Clamp sampling coordinates
        sample_coord.x = clamp(sample_coord.x, 0, int(push_constants.screen_width) - 1);

        float2 uv = float2(sample_coord) / float2(push_constants.screen_width, push_constants.screen_height);

        // Gaussian weight
        float weight = exp(-0.5 * (dx * dx) / sigma2);
