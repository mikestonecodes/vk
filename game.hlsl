...
static const float BODY_INV_MASS[3] = {
    0.0f,
    1.0f,
    1.0f
};
...
static const float BODY_ATTRACTION[3] = {
    0.0f,
    0.0f,
    42.0f * 0.5f
};

static const float BODY_MAX_SPEED[3] = {
    0.0f,
    42.0f,
    42.0f * 1.3f
};
...
BodyInitData init(uint type) {
    ...
    BodyInitData data;
    data.radius = BODY_RADIUS[idx];
    data.inv_mass = BODY_INV_MASS[idx];
    return data;
}
...
void update(uint id, float dt) {
    ...
    switch (type) {
        case 1u: {
            ...
            float target_speed = BODY_MOVE_SPEED[1u];
            if (speed > 1e-5f) {
        ...
        case 2u: {
            ...
            vel += accel_dir * (BODY_ATTRACTION[2u] * dt);
            float speed = length(vel);
            float max_speed = BODY_MAX_SPEED[2u];
            if (speed > max_speed) {
